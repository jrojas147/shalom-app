...
import { PagoComprobanteData } from '../models/pago-comprobante.model';

@Injectable({ providedIn: 'root' })
export class PagoComprobantePrintService {
  imprimir(data: PagoComprobanteData): void {
  ...
  private printWithIframe(html: string): void {
    ...
    iframe.setAttribute('title', 'Impresión comprobante de pago');
  ...
  private buildDocument(data: PagoComprobanteData): string {
...
<head>
  <meta charset="utf-8" />
  <title>Comprobante pago ${this.escapeHtml(data.factura)}</title>
  <style>${this.styles()}</style>
</head>
<body>
  <div class="ticket">
    <div class="ticket__center ticket__title">COMPROBANTE DE PAGO</div>
    ...
    <div class="ticket__row"><span>Fecha pago</span><span>${fecha}</span></div>
    <div class="ticket__row"><span>Hora</span><span>${hora}</span></div>
    <div class="ticket__row"><span>Estado</span><strong>PAGADO</strong></div>
    ...
    <div class="ticket__line">${this.escapeHtml(data.beneficiarioNombre)}</div>
    ${
      data.beneficiarioDocumento
        ? `<div class="ticket__muted">Doc: ${this.escapeHtml(data.beneficiarioDocumento)}</div>`
        : ''
    }
    ${
      data.sucursalNombre
    ...
    ${
      data.anticipoAplicado && data.anticipoAplicado > 0
        ? `<div class="ticket__row"><span>Total compra</span><strong>${this.formatMoney(
            data.totalCompra ?? data.total + data.anticipoAplicado
          )}</strong></div>
    <div class="ticket__row"><span>(-) Anticipos</span><strong>${this.formatMoney(data.anticipoAplicado)}</strong></div>`
        : ''
    }
    <div class="ticket__row ticket__total"><span>TOTAL PAGADO</span><strong>${this.formatMoney(data.total)}</strong></div>

    <div class="ticket__divider"></div>
    <div class="ticket__center ticket__footer">Comprobante de retribución</div>
  </div>
  <script>
  ...
  private styles(): string {
    return `
      ...
      * { box-sizing: border-box; }
      ...
      .ticket__muted { color: #222; font-size: 10px; word-break: break-word; }
      ...
      .ticket__table { width: 100%; border-collapse: collapse; table-layout: fixed; font-size: 10px; }
      .ticket__table th, .ticket__table td { vertical-align: top; padding: 2px 0; }
      .ticket__table th { text-align: left; font-size: 9px; border-bottom: 1px solid #000; padding-bottom: 3px; }
      .ticket__table th.num, .ticket__table td.num { text-align: right; white-space: nowrap; }
      .ticket__table th:nth-child(1), .ticket__table td:nth-child(1) { width: 34%; }
      .ticket__table th:nth-child(2), .ticket__table td:nth-child(2) { width: 28%; }
      .item-name { word-break: break-word; font-weight: 700; }
      .item-precio { font-size: 9px; color: #000; margin-top: 2px; }
      .item-empaque { word-break: break-word; font-size: 10px; font-weight: 700; color: #000; }
